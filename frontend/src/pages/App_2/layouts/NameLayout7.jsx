...
import Layout7SVG, { LAYOUT7_BOXES } from "./Layout7SVG";

const COLORS = ["#FE47C6", "#70FE99", "#7A5BFE"];

const getLetterColor = (index) => COLORS[index % COLORS.length];

function ResponsiveLetter({ letter, box, color, yOffset = 0 }) {
  ...
  const centerY = box.y + box.h / 2 + yOffset;
...
export default function NameLayout7({ name = "" }) {
  const letters = name.toLowerCase().split("").slice(0, LAYOUT7_BOXES.length);
